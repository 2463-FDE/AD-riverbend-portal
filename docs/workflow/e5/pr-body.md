...
> Status: **CLOSED UNMERGED 2026-08-13** (PR #76, owner decision; was: PUSHED PR #76
> 2026-08-12). The branch's foundations predate the current staged workflow, and eight
> review rounds with a growing accepted-residual set showed design being discovered in
> review rather than checked before it. The work restarts as successor item **e5b**, with
> this PR's findings, residuals, owner decisions and verification evidence harvested as
> spec inputs. Consequences on `main`: the underlying defect — `POST /intake` is not
> idempotent per submission attempt — is **OPEN** (`docs/debt-log.md`, the register-first
> residual under "Intake contract break"); chunk 1 (PR #74) is merged and untouched; the
> ninth codex review on the PR was never dispositioned and is recorded standing open
> (`findings.md` §Review round 9). Branch `fix/e5-registration-idempotency` is kept as a
> reference until the e5b spec freezes, then deleted. Everything below this line is the
> delivery record exactly as it stood at close — its present-tense claims describe the
> closed branch, not `main`.
>

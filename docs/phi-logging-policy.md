...
| `services/intake-service/app.py` registration-idempotency log sites (`e5b`) — `submission_id` on the `log_metadata` allowlist, plus the collision / replay-served / replay-mismatch / key-guard lines | **REVIEWED 2026-08-13** | `submission_id` is **non-PHI by construction** (e5b-SPEC-18/20): a mint-random version-4 UUID, drawn from a CSPRNG independent of all patient data, so logging it is what makes a replay diagnosable without echoing any demographic or insurance value — it joins the allowlist for that reason (e5b-D-10). The new log lines carry only that id and exception **class names** (rule 3 idiom), never a payload value. The persisted content binding is a **keyed HMAC-SHA256**, never a plain hash: an unkeyed hash of guessable fields would be a dictionary-reversible offline confirmation oracle (e5b-SPEC-21/D-8), so the fingerprint is derived from PHI and must stay keyed forever — it is never logged. Tests: `tests/test_intake_idempotency.py::test_no_phi_on_any_surface`, `::test_binding_is_keyed_not_reversible`. |

...
5. **Free-text rule (visit-chat, ADR 0011).** The chat `message` is the only
   ...
   - **Never logged.** Chat logging goes through
     ...
     enum, derived status, turn count, and the turn's report fields: mode,
     reason, outcome, model-call count, correlation id — each a closed enum, an
     integer, or a UUIDv4). Never the message, never the transcript.
   - **Never sent to the vendor.** Intent derivation and the eligibility lookup
     ...
     What crosses the boundary is two closed-vocabulary payloads plus vendored
     synthetic corpus text, pinned by `tests/test_a1_prompt_boundary.py` (ADR
     0019), which catches any future interpolation of the message.
...
| `services/ai-assistant/app.py` — `str(e)` on LLM-path errors: the five `intake_instructions` branches (`LLMConfigError`, `LLMUnavailable`, `LLMResponseError`, `LLMBudgetExceeded`, the `LLMError` catch-all) and the visit-chat degrade branch — since the agent-path rewire, `agent_turn.run_agent_path`'s reason branch, which carries the same class-only idiom and the same `request_id` attribute | **FIXED 2026-08-09** | The only vendor-egress service. An SDK error string can echo response-body fragments (which may derive from PHI-bearing prompts, D13); the `LLMError` catch-all logged the class name **and** `str(e)`. All six now use the class-only idiom (`CLAUDE.md` §4). **Two of the six were absent from this row until the W1 sweep found them** — the `LLMBudgetExceeded` branch and the visit-chat degrade — so the register was under-reporting its own subject (W1-SPEC-15). Cited by handler + exception class, not by line number, since the fix itself moves the numbers; both handler names are real symbols in the service (`intake_instructions` in `app.py`, `run_agent_path` in `agent_turn.py`), so the citation resolves by grep. Tests (red pre-fix, scan the formatted record): `tests/test_ai_intake_instructions.py::test_llm_error_log_carries_no_exception_message` over all five intake branches, `tests/test_visit_chat_phi.py::test_degrade_log_carries_no_exception_message`. **Two of the six also log the provider `request_id`** (the `LLMResponseError` intake branch and the degrade branch, 2026-08-10): it is the same non-PHI correlation field the success log emits, carried as a structured `LLMError.request_id` attribute rather than read out of the message, so rule 3 holds while a schema-drift incident stays traceable to the vendor's own logs (Codex PR #69 round 1). Tests: `::test_bad_response_log_carries_request_id_and_not_the_message`, `::test_degrade_log_carries_the_provider_request_id`. |
...
| Redis holds `facts.insurance_id` at rest (visit-chat) — and, since the turn's citation provenance landed, `facts.last_citations` (document ids + versions only, never text; a citation is re-rendered from the index by id) | **ACCEPTED 2026-07-26; amended 2026-08-31** | Approved under rule 6's controls (opaque key, owner binding, sliding TTL, no id in keys/logs); `last_citations` sits under the same controls on the same key. The Redis hardening that was its recommended precondition shipped with it (**D3b**, PR #14): no host publish, `requirepass`, scoped credential, gateway-side fail-closed guard. Residual: no TLS, one shared credential, no read audit trail. |
| `services/intake-service/app.py` registration-idempotency log sites (`e5b`) — the `submission_ref` digest on the initial-metadata line, plus the collision / replay-served / replay-mismatch / key-guard lines | **REVIEWED 2026-08-13; digest hardening 2026-08-14** | The `submission_id` is minted non-PHI by the portal (e5b-SPEC-18: a CSPRNG version-4 UUID independent of all patient data), but the **service cannot prove that** — a caller reaching the gateway directly bypasses the mint, and the version-4 format check narrows accidental derivation without proving randomness (e5b-D-9). A well-formed v4 UUID still carries 122 caller-controlled bits, enough to smuggle an SSN/MRN into its hex digits, so logging the raw value would make this PHI-log boundary depend on the portal being the only minting authority (PR #79 codex r1). **All log sites therefore emit a `REGISTRATION_FINGERPRINT_KEY`-keyed HMAC digest (`_submission_log_id` → `submission_ref`), never the raw id** — replays stay correlatable, no caller-embedded identifier survives. The key is checked real before any request-path log site, so a digest is never taken over a weak key. Lines carry only that digest, `patient_id`, and exception **class names** (rule 3 idiom), never a payload value. The persisted content binding is a **keyed HMAC-SHA256**, never a plain hash: an unkeyed hash of guessable fields would be a dictionary-reversible offline confirmation oracle (e5b-SPEC-21/D-8), so the fingerprint is derived from PHI and must stay keyed forever — it is never logged. Tests: `tests/test_intake_idempotency.py::test_no_phi_on_any_surface`, `::test_malicious_valid_uuid_is_never_logged_raw`, `::test_binding_is_keyed_not_reversible`. |

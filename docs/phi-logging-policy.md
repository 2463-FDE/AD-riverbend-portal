...
| `services/gateway/app.py` login handler `str(e)` DB-error path | OPEN | `log.error("login db error: %s", e)` on the users SELECT — same rule-3 class; a statement-level error can embed the attempted username via the driver message. In the auth zone (CLAUDE.md §6), so the one-line idiom fix is approval-gated and was deliberately not folded into the 2026-08-05 intake fix. Found by the pre-push adversarial review of that fix. |

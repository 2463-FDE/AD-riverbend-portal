...
| Engine-level backstop | **ADDED 2026-08-05** | `hide_parameters=True` on every service `create_engine` (gateway, intake, records, roi, scheduling — interop/eligibility have no engine), so even an unfixed future `str(e)` site cannot embed bound rows. |

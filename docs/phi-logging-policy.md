...
| Site | Status | Notes |
|------|--------|-------|
| `services/intake-service/app.py:67` full body at INFO | **FIXED 2026-07-08** | Now logs allowlisted metadata (`schemas.log_metadata`), body never logged. Interim `safe_log_payload(req)` (2026-07-05) still leaked names/DOBs via the open `consents` list — pattern scrub misses them; `consents` is now a `ConsentKind` enum (Codex review). **The projection gained one copied-out value 2026-08-11** (`e5` chunk 2): `submission_id`, the caller's identifier for the submission attempt. It is the same shape of non-PHI correlation field as the LLM path's provider `request_id` — random, derived from no submitted value (E5-SPEC-38), and the thing that makes a lost-confirmation retry traceable at all (E5-SPEC-39). Every other entry stays a boolean presence flag or the `ConsentKind`-constrained list. Tests (scan the stored row, the formatted log records and the response for every submitted value): `tests/test_intake_idempotency.py::test_the_identifier_carries_no_submitted_value_anywhere_it_lands`, `::test_the_log_projection_carries_the_identifier_and_no_new_value`. **What enforces the "derived from no submitted value" claim, and what does not** (PR #76 review, 2026-08-11): the mint is the portal's (`frontend/app/intake/payload.ts`); the service additionally rejects any `submission_id` that is not a version 4 UUID, which closes the name-derived v5 case — the accidental way patient values reach this projection — but a version check is not proof of randomness, so a caller determined to log a derived value could still stamp v4 bits on one. Read this row as "the logged value is non-PHI because the only caller mints it at random, and the accidental derivations are rejected at the boundary", not as "the boundary proves it." Tests: `tests/test_intake_schemas.py::test_a_uuid_that_is_not_v4_is_rejected`. **A second value now reaches persisted state 2026-08-11** (`e5` chunk 2, review round 2): `registration_submissions.payload_fingerprint`, a keyed HMAC-SHA256 of the submitted payload (E5-SPEC-41). It is PHI-DERIVED and therefore kept keyed — a plain hash of DOB/SSN/member id would be a dictionary-reversible confirmation oracle over a column — and the service fails closed (503) rather than fingerprinting with no `REGISTRATION_FINGERPRINT_KEY` set. It is never logged, and the 409 a content mismatch answers with carries a constant detail. Tests: `tests/test_intake_idempotency.py::test_the_fingerprint_is_keyed_and_reveals_no_submitted_value`, `::test_the_stored_fingerprint_carries_no_submitted_value`, `::test_a_recorded_identifier_with_different_content_is_refused`. **"Keyed" now means keyed with a real secret 2026-08-12** (PR #76 review round 3): the guard was presence-only, and this branch had shipped a non-empty placeholder in `.env.example` that CI copies into `.env` — so a deploy seeded from the template would have computed these fingerprints under a value committed to this repo, which is the dictionary-checkable case the keying exists to prevent. `app.py::_fingerprint_key` now also refuses known placeholder sentinels and any key under 32 characters, the template ships the key EMPTY (the `AWS_BEARER_TOKEN_BEDROCK` pattern, `llm_client::_require_bearer_token`), and the refusal names the variable only — never the value. **Where the key lives changed 2026-08-12** (PR #76 review round 5): emptying the shared template made a template-seeded stack answer 503 to every registration while `/healthz` — which knew nothing about the key at the time — kept the container green. The key moved to a scoped `.env.registration` loaded by intake-service alone and **generated** per machine by `make up` (the `.env.redis` pattern), so the fail-closed guard no longer costs availability, and a PHI-derived key no longer rides the shared `.env` that reaches every container on the network. Tests: `tests/test_intake_idempotency.py::test_a_placeholder_or_too_short_key_is_refused_exactly_like_no_key` (8 cases), `::test_the_key_the_template_ships_fails_closed` (reads `.env.registration.example`, so it reddens if a usable-looking value returns), `::test_the_generated_key_registers` (runs the Makefile recipe and feeds what it wrote to the guard), `::test_the_refusal_never_carries_the_key`, `::test_a_real_key_registers` (the positive control), plus the scoping and generation pins in `tests/test_compose_topology.py`. **The key gained a second refusal site 2026-08-13** (PR #76 review round 7): `/healthz` now asks the same predicate (`_fingerprint_key_is_real`, shared with `_fingerprint_key` so the two askers cannot diverge) and answers `503 registration key not configured`, because a container reporting healthy while it can register nobody is the failure round 5's generation made unlikely rather than impossible. Round 5 had declined this on the grounds that a `/healthz` computing a fingerprint is a PHI-adjacent probe; the landed check reads configuration only and computes no digest, and the log line is polled every 10 seconds, so it names the variable and states neither the value, its length, nor which check refused. Tests: `tests/test_intake_schema_guard.py::test_healthz_goes_red_when_the_key_is_not_a_real_secret` (5 cases), `::test_the_key_red_discloses_no_key_material`. The schema half of the same endpoint reads catalog metadata only and selects no row — `::test_the_red_names_the_missing_table_and_reads_no_row` plants a patient and asserts none of it reaches the log or the body. |
...
| `services/intake-service/app.py` `_create_registration` error path | **FIXED 2026-08-05** | Three separate write helpers when the finding was made, one transaction since 2026-08-10 (`e4`); the fix is inherited by the merged helper, not re-made, and the blast radius each row recorded still describes what its write binds. **Patient write:** logged `str(e)` on `SQLAlchemyError`; a statement-level `DBAPIError` (e.g. `DataError` on an oversized field) embeds `[parameters: (...)]` — the full patients row: name, DOB, SSN, address, notes (rule 3). **Coverage write:** same class; would have embedded `member_id` / `group_number` (rules 2 and 3). **Consent write:** same `str(e)` pattern; bound values were benign consent kinds, fixed for class completeness (rule 3) — and its error is no longer swallowed either, so the failure now reaches the caller as a 503 instead of a silent partial registration (E4-SPEC-4). All log the exception class only (the `_verify_eligibility` idiom). Test: `tests/test_intake_db_error_phi.py` (red pre-fix). Found by doc-drift follow-up 2026-08-05. |
...
| `services/ai-assistant/app.py` — `str(e)` on LLM-path errors: the five `intake_instructions` branches (`LLMConfigError`, `LLMUnavailable`, `LLMResponseError`, `LLMBudgetExceeded`, the `LLMError` catch-all) and the visit-chat degrade branch in `_reply_items` | **FIXED 2026-08-09** | The only vendor-egress service. An SDK error string can echo response-body fragments (which may derive from PHI-bearing prompts, D13); the `LLMError` catch-all logged the class name **and** `str(e)`. All six now use the class-only idiom (`CLAUDE.md` §4). **Two of the six were absent from this row until the W1 sweep found them** — the `LLMBudgetExceeded` branch and the visit-chat degrade — so the register was under-reporting its own subject (W1-SPEC-15). Cited by handler + exception class, not by line number, since the fix itself moves the numbers; both handler names are real symbols in that file (`intake_instructions:223`, `_reply_items:714`), so the citation resolves by grep. Tests (red pre-fix, scan the formatted record): `tests/test_ai_intake_instructions.py::test_llm_error_log_carries_no_exception_message` over all five intake branches, `tests/test_visit_chat_phi.py::test_degrade_log_carries_no_exception_message`. **Two of the six also log the provider `request_id`** (the `LLMResponseError` intake branch and the degrade branch, 2026-08-10): it is the same non-PHI correlation field the success log emits, carried as a structured `LLMError.request_id` attribute rather than read out of the message, so rule 3 holds while a schema-drift incident stays traceable to the vendor's own logs (Codex PR #69 round 1). Tests: `::test_bad_response_log_carries_request_id_and_not_the_message`, `::test_degrade_log_carries_the_provider_request_id`. |
...
| `services/gateway/app.py` `_post`/`_get` proxy helpers — deleted with their last call site | **FIXED 2026-08-11** | Log `str(e)` and return `{"error": str(e)}` on any outbound failure — rule 3's outbound flavor, in the known error-swallowing proxy debt (D4; CLAUDE.md §4 "do not imitate"). Measured mitigation: installed httpx renders `ConnectError` without the URL, so the common failure carries no query params; URL-bearing exception types are edge paths. Fix belongs to the D4 `_post_checked` migration, not a log patch. **`POST /intake` migrated 2026-08-10** (`e4`, owner-approved): that route now uses `_post_checked`, which logs the exception class only and relays a downstream `detail` solely when it is a plain string — so a FastAPI 422, whose `detail` is a list carrying the rejected `input` values, cannot reach the caller. Test: `tests/test_gateway_intake_proxy.py`. **The remaining thirteen call sites were converted 2026-08-11 (`e5`, owner-approved) and both helpers were DELETED**, so this log site no longer exists rather than merely having no callers. Every gateway fan-out now goes through `_get_checked`/`_post_checked`, which log the exception class only. Tests: `tests/test_gateway_proxy_error_contract.py` — a poisoned-URL case over all thirteen routes scanning both the response and every gateway log record, plus a structural scan (with a positive control) that no such helper is defined or called. Not covered by this row and NOT fixed by `e5`: the `httpx` library itself logs each outbound request URL, query string included, at INFO — see `docs/workflow/e5/pr-body.md`, filed for the owner to route rather than fixed inside an error-contract change. |

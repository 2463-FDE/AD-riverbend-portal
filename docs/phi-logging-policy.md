...
| `services/gateway/app.py` login handler `str(e)` DB-error path | **FIXED 2026-08-05** | Logged `str(e)` on the users SELECT — same rule-3 class; a statement-level error can embed the attempted username via the driver message. Auth zone (CLAUDE.md §6): folded into the intake DB-error fix on explicit approval. Now class name only. Test: `tests/test_gateway_login_db_error_phi.py` (red pre-fix). Found by the pre-push adversarial review of that fix. |

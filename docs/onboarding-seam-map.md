...
| **Inline synchronous outbound calls on request threads** (D4) | Intake's eligibility call — the "spinning registration" (RIV-088/141) — is bounded since PR #11 / ADR 0010: per-hop timeout + in-process circuit breaker, and an outage returns `pending`/`unknown`, never a false `inactive`. Still inline on the `/intake` request thread, and breaker state is per worker, so the wall stands. Do not add more inline outbound calls; every new outbound call gets a timeout (see `ai-assistant/llm_client.py` for the pattern). Do not widen a timeout or loosen a breaker threshold without re-reading ADR 0010 — inner and outer values are pinned to each other, enforced by `tests/test_eligibility_budget_alignment.py`. |
...
| **`.env` secrets in git history** | `.env` is gitignored and untracked now, but it *was* committed and the old secrets remain in git history (rotation + scrub pending). CI's `gitleaks` runs `--no-git`, so it catches recurrence in the tracked tree, not history. Never add secrets to `.env`; placeholders go in `.env.example`. Flag before rotating. |

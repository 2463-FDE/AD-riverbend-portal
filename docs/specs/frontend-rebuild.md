...
| 2 | Framework: SvelteKit vs stay on Next.js. Wins for Svelte are form ergonomics, build-time a11y enforcement, native scoped CSS, no RSC ceremony, differentiation. The one serious cost is continuity of the client's stack. Bundle size is **not** an argument here (2.3k lines; the felt latency is the D4 eligibility hop). **Write P0's requirements before scoring, not after.** | G1 | G0 output + ADR |
...
| 4 | New frontend as a second compose service (recommended — satisfies `FE-R15` literally) vs a route group behind a flag. | P2 | user call |

...
| `FE-R34` | The day queue shall exclude cancelled visits, and shall not omit a visit on the basis of a status it does not recognise. **[Backend contract, stated here because §8 #7's endpoint has no other spec home — added 2026-08-02, codex PR #26 r2.]** | **CI:** `tests/test_schedule_day_view.py` asserts the compiled statement carries `status != :bind` bound to `'cancelled'` and carries no `status IN (…)`. Mutation-proven three ways: the allowlist variant, no predicate at all, and a cancel path storing a different spelling — each red on the test that guards it | — | G5 |
...
| `FE-R15` | The repository shall retain the original portal as a separately runnable service until the new portal reaches feature parity at **G5**, **and additionally until P7 re-homes the intake-instructions UI (G6)**. **[Condition widened 2026-08-01.** It read "until `FE-R1`–`FE-R3` pass", which close at **G2** — login plus the minimum intake path. Records land at G3 and the appointment/ROI queues at G5, so the original wording released the old portal three gates before anything replaced what it does. Phrased as a floor it forced no premature deletion, but it was the only stated condition and reads as permission.**]** **[Widened again 2026-08-02:** G5 parity does not cover W1's intake-instructions checklist, whose only UI is legacy (`frontend/app/intake/page.tsx:141`) and whose new home is **P7, after G5** (§8 #17) — so retiring legacy at G5 would regress a met client ask ("get something on the screen") to headless for the P7 window. Retirement condition is therefore G5 parity **plus** the P7 re-home; the rest of P7 (W3's first-ever surface) adds nothing legacy has and does not hold the old portal.**]** | `make config` + both services up, re-checked at each gate from G2 through G6 | — | G2 |
...
| **G4** | P5 Role-aware shell | — | role model decision | **explicit human approval for an auth change (CLAUDE.md §6)**. The backend half shipped: ADR 0017 landed the role set in `config/roles.yaml`, seeded assignment and gateway `require_capability` enforcement — with **no schema change** (`users.role TEXT DEFAULT 'staff'` already existed; only seeded values moved, so the earlier claim here that a `db/schema.sql` edit + hand-synced migration were needed was wrong). G4 verifies the portal's role-aware surface against those real roles | user, explicitly |
...
| 18 | **W4's client ask has no screen in any spec — found 2026-08-02, closing the audit gap #17 left.** #17 checked the P2 mockups against W1–W3 only; the W4 ask — "assembles a patient's full picture in one view … make it feel fast" — never got the same check, and it is the same headless pattern: `w4.md` deliberately decodes the ask into an IDOR writeup plus a prototype (building a feature on `GET /patients/{id}/records` as-is would propagate D11), so no document commits the assembled view to a screen, and the caller in the client's own quote — a patient — is served by neither spec. Three parts, resolved separately. **(a) Interim answer — decided 2026-08-02 (user):** the G3 staff chart surface (`FE-R4`–`R7`, `FE-R25`) is the "one view" for now, presented to patients through staff, and this row is the document that says so out loud; the patient-facing half stays behind #16's D11 blocker and adds nothing new here. **(b) Open — does the G3 chart consume W4's assembly output** (probe call, reduced record set, gap facts) **or raw records-service reads?** Not decidable yet: `w4.md` §7 Q5 (where the agents run) is unresolved and the probe endpoint does not exist (Q6), so the assembly may have nothing consumable when G3 is designed. Decide at G3 design; if the answer is raw reads, W4's D8 findings (batched fetch, scoped+indexed search) still bind the chart's data layer — bounded reads are the "make it feel fast" half of the ask and they do not depend on the agent question. **(c) Open — the ask's actual nouns, labs and visit summaries, have no FE-R:** the chart body's requirements are header (`FE-R4`) and allergies (`FE-R5`) only, the rest lives unrequirement'd in `docs/design/04-wireframes.md`. Write the chart-body FE-Rs when G3 is designed — same design-first discipline as P7, not here. | (b), (c) block G3 design sign-off | (a) resolved; (b) G3 design + `w4.md` §7 Q5/Q6; (c) G3 design |

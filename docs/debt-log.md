...
|------|---------------|--------|
| No migration runner — a database that predates a migration never receives it | `db/migrations/*.sql` is an ordered history with nothing that applies it, and `docker-compose.yml:12` mounts only `db/schema.sql` into `/docker-entrypoint-initdb.d`, which Postgres runs on a **fresh volume only**. A `pgdata` volume created before a migration therefore lacks its table, and the first service to read that table catches the `SQLAlchemyError` and answers **503 on every request through that path**, not just the new one — an availability failure that looks like an outage, with a healthy container reporting it. **Measured 2026-08-13** (PR #76 round 6, scratch container seeded from `main`'s schema): `POST /intake` against a volume without `registration_submissions` fails at `services/intake-service/app.py:372-383`. The class is older than e5 — `insurance_coverages` (mig 005), `roi_requests` (008) and `duplicate_review_queue` (009) are read the same unconditional way, so any environment older than those migrations has the same latent break. **Partly mitigated 2026-08-13, not closed:** `make schema-apply` re-applies the flattened schema (every table is `CREATE TABLE IF NOT EXISTS`, verified to create the missing table and its constraint while leaving `patients` at 255 rows), `docs/runbook.md` §"Upgrading a database that predates a migration" is the operator entry, and `tests/test_schema_upgrade_path.py` pins the re-appliable form plus the table/column hand-sync. **Detection added 2026-08-13** (round 7, owner overruling round 6's "operator path only"): `intake-service`'s `/healthz` refuses with `503 schema incomplete` when any table it maps is absent, so the compose healthcheck turns the condition into an **unhealthy container** instead of a green one answering `503` to every registration (`tests/test_intake_schema_guard.py`). The check is over `Base.metadata`, so a future model is inside it without an edit — but it is **one service's**: `records-`, `roi-` and `scheduling-service` still present this condition as request-path `503`s behind a healthy container. **What stays open:** `IF NOT EXISTS` skips an existing table whole, so an `ALTER TABLE ... ADD COLUMN` migration is still hand-applied; nothing records which migrations a database has had; nothing in `make up` or CI notices the mismatch; and the four other services have no equivalent guard. Closing it is a real runner (Alembic or a versions table + apply step) — estate-wide, inside the `docs/landmines.md` §1 migrations zone, and out of scope for the item that measured it (owner decision 2026-08-13) | OPEN — mitigation shipped, runner unscheduled |
| `make seed` doubles the demo data on a database that already has it | `db/seed/seed.sql` (generated, 2507 lines) carries **no `ON CONFLICT` clause anywhere**, so re-running it against a populated database errors on the inserts that name their ids (`users`, `patients`, `encounters`, `records`, `slots` — skipped) and silently duplicates every table whose ids are serial. **Measured 2026-08-13** (exact counts, before → after a second run): `consents` 403→806, `insurance_coverages` 255→510, `appointments` 209→418, `roi_requests` 16→32, `audit_logs` 22→44, `disclosures` 8→16, while `patients` stayed at 255. That split is the sharp edge — the duplicated coverage, consent and appointment rows attach to the **original** patients, so the corpus ends up internally inconsistent rather than merely twice as large. Until 2026-08-13 `docs/runbook.md:23-29` advertised the command for exactly this case ("reload demo data into an already-running DB"), which is how a duplicated corpus gets created deliberately — and duplicate charts are the subject of D5a, so a doubled seed also corrupts the fixture that teaches it. The runbook entry and the `Makefile` `##` line now say fresh-DB-only and point at `make schema-apply`; the target itself is unchanged, so the foot-gun is documented rather than removed. Fix if taken: `ON CONFLICT DO NOTHING` in the generator, or a target that refuses a non-empty database | OPEN — documented 2026-08-13, behaviour unchanged |

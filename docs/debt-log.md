...
| D5a | `services/intake-service/app.py:69` | No MPI → duplicate charts per patient; clinical data (e.g. allergies) splits across charts — patient-safety risk. **Narrowed 2026-08-08 (W2, ADR 0005 Accepted):** candidate flagging is live — intake evaluates a tier-1 match key (normalized SSN + corroborating demographics) at chart-create, queues candidate pairs for front-desk review (`duplicate_review_queue`), and a retroactive pass (`retro_match.py`) covers rows created before it; a clinician opening a flagged chart gets a disclosure banner. **Nothing merges** — dispositioning records a human judgment and the merge itself stays a manual HIM procedure, so today's split charts stay split and the fragmentation risk is disclosed, not removed. Tier 2 (fuzzy name + DOB where the SSN is missing or invalid) is **deferred**, so duplicates without a usable SSN are still undetected — that is W2's detection floor | RIV-160 | OPEN (detection live, remediation manual) |
| D5b | `services/scheduling-service/app.py:98` | Check-then-insert booking race, no UNIQUE on `slot_id`, no idempotency key → double-booked slots, "charged twice" complaints | RIV-175 | OPEN — **note: the seeded markers reuse "D5" for both this and D5a**; disambiguated here as D5a/D5b. **Second, independent path found 2026-08-06:** nothing anywhere writes `Slot.status` (grep finds only the read filter `Slot.status == "open"`, `scheduling-service/app.py:49`), so a confirmed booking leaves its slot `open` forever and `GET /slots` keeps offering it. Two users minutes apart both see it and both book — no race required, so a UNIQUE constraint alone does not close RIV-175. Also note `book.py` `slot_taken`/`insert_appointment` closed `conn` only on the success path, leaking a connection on any exception — **closed by e6 2026-08-16** (try/finally release, e6-SPEC-7); the check-then-insert race, the missing `slot_id` UNIQUE and the missing idempotency key are untouched (e6-SPEC-8, RIV-175 stays seeded) |
| D5c | `services/intake-service/app.py` (replay branch, e5b) | **Signal-less match-evaluation gap on a crash-window registration.** Duplicate matching runs post-commit by design (W2-SPEC-23/32, never blocks a registration), and e5b's idempotent replay answers a recorded `submission_id` without re-running it. A process death between the registration commit and `_evaluate_match_key` therefore leaves a patient unevaluated with **no `MatchEvaluationFailure` marker** — unlike a matcher fault, nothing surfaces it in the retro pass summary's "still unevaluated" counter. Recovery mechanisms exist and cover it: the retroactive pass (`retro_match.py`) is a marker-independent full sweep over every patient, and any later registration sharing the SSN re-derives the whole candidate clique. But the retro pass is **operator-run only** (runbook §"Retroactive duplicate-match pass"; no cron, compose service, CI job, or Makefile target), so recovery is unprompted. Mitigation until scheduled: run the retro pass periodically, not only for pre-match-key backfill. Filed from PR #79 codex round 4 (e5b) | — | OPEN — window is narrow (single-request crash mid-intake); detection posture otherwise per D5a |
...
| D11 | `services/records-service/app.py:91` | IDOR: sequential integer `patient_id` served to any logged-in user; sessions not patient-bound — cross-patient chart reads succeed. **Exposure was larger than id-walking (measured 2026-08-06; corpus-read vector closed by e6 2026-08-16):** `q` was interpolated into the search pattern un-escaped at `records-service/app.py:339` (search) and `:51-53` (patient-name filter), so `GET /records/search?q=%25` (a bare `%` wildcard) matched every row and returned full `Record.body` text for all of them, with no `LIMIT` — one request, whole corpus, no ids required. e6 escapes the LIKE metacharacters at both sites and bounds the result set (e6-SPEC-1/2/5), closing that vector. The IDOR itself is untouched: sessions are still not patient-bound and id-walking still succeeds — that is the remaining D11 fix (W4, TODO-20). **Exposure set grew 2026-08-08 (W2):** `GET /patients/{patient_id}/relevant-records` (`records-service/app.py`, gateway `:332`, `records.read`) is a second per-patient chart-read route with the same session-not-patient-bound property — inherited in kind, not widened in kind (no new capability, no new cross-patient query surface, no search parameter). **Exposure set re-sized 2026-08-17 (w4, `docs/research/w4-findings.md`):** the D11 fix is now sized against a **classification-complete sweep of all 23 gateway `@app.` routes** — the swept 23-route table in the findings doc, pinned by `tests/test_w4_exposure_sweep.py`, not an example list that three impl-gate rounds proved could not stay complete. The **13 in-set routes**: the three per-patient chart reads (`GET /patients/{patient_id}` returning `PatientDetail` — plaintext `ssn`, `dob`, `address`, `records-service/schemas.py:18-32`; `GET /patients/{patient_id}/records`; `GET /patients/{patient_id}/relevant-records`), the caller-selected reads and writes outside records-service (`GET /appointments?patient_id=` and its write twin `POST /appointments/{appointment_id}/cancel`, `scheduling-service`; `POST /appointments`; `POST /roi/requests` and `POST /roi/requests/{request_id}/fulfill`; `POST /review-queue/{pair_id}/disposition`), and the four cross-patient-by-construction routes (`GET /patients`, `GET /records/search`, `GET /roi/requests`, `GET /review-queue` — candidate duplicate pairs with name + dob across all patients). Several — `GET /patients/{patient_id}`, the scheduling surfaces, and the `GET /review-queue` + `POST /review-queue/{pair_id}/disposition` pair — were on neither this row nor `docs/landmines.md` §1 before w4 swept them. The fix is therefore not records-service-local: a session→patient bind has to hold **at the gateway** to cover the scheduling, ROI and review-queue surfaces too. Any route added later is caught by the sweep test's route-set closure, not by a fourth manual catch | — | OPEN (xfail test in suite) |
| D13 | `services/ai-assistant/` (both endpoints); `adr/0004`, `adr/0006`, `adr/0009`, `adr/0011`, `docs/phi-logging-policy.md` | **PHI to a cloud LLM vendor with no BAA.** Riverbend is a covered entity, so any prompt may carry PHI, and the assistant runs against Bedrock on standard SaaS terms — no executed Business Associate Agreement covers it. `/ai/*` is the estate's only vendor-egress path, which is why every AI ADR treats D13 as the constraint it designs around: `/intake-instructions` is safe by construction (closed-vocabulary enums/bools, ADR 0004) and `/visit-chat` never egresses clerk prose (ADR 0011/0019; two closed-vocabulary payloads plus vendored synthetic corpus text, pinned by `tests/test_a1_prompt_boundary.py`). So the boundary holds *today by construction*, not by contract — a future feature that interpolates free text into a prompt breaches it with no gate to stop it. `adr/0009-ai-assistant-bedrock-provider.md:21-33` names the closing mechanism: Bedrock is HIPAA-eligible under an executed AWS BAA with a BAA-covered region/model, so routing under BAA closes this rather than widening it. **Row added 2026-08-07** (W3 backfill): D13 gated a whole path and was cited in five ADRs, a policy doc and two code comments while having no entry in this register | — | OPEN — precondition for any real-PHI traffic; scenario-scheduled for W8 |
...
| D16 | `services/intake-service/models.py:107`, `db/migrations/010_registration_submissions.sql` | **Caller-controlled `submission_id` stored raw in a never-pruned ledger.** The idempotency ledger keys on the portal's mint-random v4 UUID and stores it raw `TEXT` under a UNIQUE constraint (e5b-SPEC-18/20/21, e5b-D-10 — non-PHI *by construction*). The construction guarantee is the portal's mint (`crypto.randomUUID`); the service validates v4 *shape* only and cannot prove randomness (the e5b-D-9 named limit). A direct API caller bypassing the portal can therefore embed an SSN/MRN in the UUID's 122 free bits, and the ledger — which never prunes — retains it in the table and every backup. This is the storage-surface twin of the PHI-smuggle vector `e5b` r1 accepted and closed on the *log* surface (keyed digest, `cf19d96`); the log fix did not touch storage, and `docs/phi-logging-policy.md` governs logs, not data-at-rest. Bounded: the portal is today's only caller and always mints randomly, so the vector requires a direct-API caller. Closing mechanism if reopened: store a server-keyed HMAC digest of `submission_id` for lookup/uniqueness, keep the raw value in request memory only — a §1 PHI-column + migration change, hence a re-gate, not a fix-session patch. Filed from PR #79 codex round 8 (e5b); owner declined to reopen frozen e5b-SPEC-18 2026-08-15 | — | OPEN — narrow (requires a non-portal caller); reopening e5b-SPEC-18 is owner-only |
...
|------|---------------|--------|
| No migration runner — a database that predates a migration never receives it | **Operator action today:** `make migrate` — the real runner, see `docs/runbook.md` §"Upgrading a database that predates a migration". Below is the pre-e6 account of the gap it closed. — `db/migrations/*.sql` is an ordered history with nothing that applies it, and `docker-compose.yml:12-13` mounts only the schema and seed into `/docker-entrypoint-initdb.d`, which Postgres runs on a **fresh volume only**. A `pgdata` volume created before a migration therefore lacks its table, and the first service to read that table catches the `SQLAlchemyError` and answers **503 on every request through that path**, not just the new one — an availability failure that looks like an outage, reported by a container that stays healthy. **Measured 2026-08-13** (PR #76 review round 6, scratch container seeded from a pre-migration schema): every request through the affected route 503s while `make ps` stays green. The class is what matters, not the instance that measured it: `insurance_coverages` (mig 005), `roi_requests` (008) and `duplicate_review_queue` (009) all arrived by migration and are read the same unconditional way, so any environment older than those migrations carries the same latent break, and every future migration joins the class. Two partial escapes exist today and both mislead: `make seed` does pipe the schema first, but then re-runs the seed (the foot-gun in the next row), and hand-piping `db/schema.sql` works (all 14 tables are `CREATE TABLE IF NOT EXISTS` — `docs/runbook.md` §"Upgrading a database that predates a migration" is the operator entry) but skips existing tables whole, so `ALTER TABLE` migrations stay hand-applied and nothing records which migrations a database has had. A fuller mitigation shape — a `schema-apply` make target, structural tests pinning the schema/migration hand-sync, and a `/healthz` schema-completeness guard that turns the condition into an unhealthy container — was built and verified on the closed PR #76 branch and is cherry-pickable from it; **none of it is on `main`**. Closing the gap outright is a real runner (Alembic, or a versions table plus an apply step) — estate-wide, inside the `docs/landmines.md` §1 migrations zone. **Closed by e6 (2026-08-16):** `db/migrate.py` (`make migrate`) is that runner — a `schema_migrations` ledger, ordered apply of `db/migrations/*.sql`, applied files never re-run (e6-SPEC-9/10), per-file atomic (e6-SPEC-11), never seeds (e6-SPEC-12), pinned structurally equivalent to `db/schema.sql` (e6-SPEC-14). `db/schema.sql` pre-stamps the ledger so a fresh volume is born current (e6-SPEC-17), and the column-add subclass (004/006/007/008) a schema.sql re-run silently skipped is included (e6-SPEC-13). A red CI `migrations` job blocks merge. **Scope:** closed for volumes born from e6 onward, and for a legacy volume that passes the verified `baseline` (stamps only after the live schema structurally matches a scratch build of the full set, e6-D-19). A behind-head legacy volume is *refused* by `baseline`; bringing it to head by hand before `baseline` is the named residual (`docs/runbook.md`) | CLOSED by e6 2026-08-16 (born-from-e6 + verified-baseline legacy volumes; behind-head legacy volume refused → manual-repair residual) |
| `make seed` doubles the demo data on a database that already has it | **Operator action today:** run `make seed` only against a fresh, empty database; to start over, `docker compose down -v` then `make up` (`docs/runbook.md` §First-boot data). — `db/seed/seed.sql` (generated, 2507 lines) carries **no `ON CONFLICT` clause anywhere**, so re-running it against a populated database errors on the inserts that name their ids (`users`, `patients`, `encounters`, `records`, `slots` — skipped) and silently duplicates every table whose ids are serial. **Measured 2026-08-13** (exact counts, before → after a second run): `consents` 403→806, `insurance_coverages` 255→510, `appointments` 209→418, `roi_requests` 16→32, `audit_logs` 22→44, `disclosures` 8→16, while `patients` stayed at 255. That split is the sharp edge — the duplicated coverage, consent and appointment rows attach to the **original** patients, so the corpus ends up internally inconsistent rather than merely twice as large. Until 2026-08-13 `docs/runbook.md:23-29` advertised the command for exactly this case ("reload demo data into an already-running DB"), which is how a duplicated corpus gets created deliberately — and duplicate charts are the subject of D5a, so a doubled seed also corrupts the fixture that teaches it. The runbook entry and the `Makefile` `##` line now say fresh-DB-only; the target itself is unchanged, so the foot-gun is documented rather than removed. Fix if taken: `ON CONFLICT DO NOTHING` in the generator, or a target that refuses a non-empty database | OPEN — documented 2026-08-13, behaviour unchanged |

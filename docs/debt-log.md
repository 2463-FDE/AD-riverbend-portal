...
| D11 | `services/records-service/app.py:91` | IDOR: sequential integer `patient_id` served to any logged-in user; sessions not patient-bound — cross-patient chart reads succeed. **Exposure was larger than id-walking (measured 2026-08-06; corpus-read vector closed by e6 2026-08-16):** `q` was interpolated into the search pattern un-escaped at `records-service/app.py:339` (search) and `:51-53` (patient-name filter), so `GET /records/search?q=%25` (a bare `%` wildcard) matched every row and returned full `Record.body` text for all of them, with no `LIMIT` — one request, whole corpus, no ids required. e6 escapes the LIKE metacharacters at both sites and bounds the result set (e6-SPEC-1/2/5), closing that vector. The IDOR itself is untouched: sessions are still not patient-bound and id-walking still succeeds — that is the remaining D11 fix (W4, TODO-20). **Exposure set grew 2026-08-17 (w4 sizing):** `GET /patients/{patient_id}` (`records-service/app.py:86`, gateway `:320`, capability `patients.read`) returns `PatientDetail` — plaintext `ssn`, `dob`, `address` (`records-service/schemas.py:18-32`) — on the same walkable sequential id with no patient bind. It was on neither this row's list nor `docs/landmines.md` §1 (which names `/patients`, `/records/search`, `/roi/requests`); added here so the D11 fix is sized against it. Same property, no new capability. Also on neither list, and outside records-service entirely: `GET /appointments?patient_id=` (`scheduling-service/app.py:71-93`, gateway `:365-371`, capability `schedule.read`) returns `AppointmentOut` — free-text `reason`, `provider`, `location`, `scheduled_for` (`scheduling-service/schemas.py:27-38`) — on the same walkable sequential id with no patient bind, and its write twin `POST /appointments/{appointment_id}/cancel` (gateway `:379`, `scheduling-service/app.py:138-139`) takes only an appointment id, so any `appointments.write` holder cancels any patient's appointment. The D11 fix is therefore not records-service-local: a session→patient bind has to hold at the gateway to cover the scheduling surfaces too. Found while sizing the exposure set for `docs/research/w4-findings.md`. **Exposure set grew 2026-08-08 (W2):** `GET /patients/{patient_id}/relevant-records` (`records-service/app.py`, gateway `records.read`) is a second per-patient chart-read route with the same session-not-patient-bound property — inherited in kind, not widened in kind (no new capability, no new cross-patient query surface, no search parameter). Listed here because `docs/landmines.md` §1 says to size the D11 fix against the whole exposure set, and a route that is not on the list does not get sized | — | OPEN (xfail test in suite) |

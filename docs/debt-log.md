...
|------|---------------|--------|
| No migration runner — a database that predates a migration never receives it | **Operator action today:** after pulling a change that adds a table, hand-apply `db/schema.sql` per `docs/runbook.md` §"Upgrading a database that predates a migration" — never `make seed`. — `db/migrations/*.sql` is an ordered history with nothing that applies it, and `docker-compose.yml:12-13` mounts only the schema and seed into `/docker-entrypoint-initdb.d`, which Postgres runs on a **fresh volume only**. A `pgdata` volume created before a migration therefore lacks its table, and the first service to read that table catches the `SQLAlchemyError` and answers **503 on every request through that path**, not just the new one — an availability failure that looks like an outage, reported by a container that stays healthy. **Measured 2026-08-13** (PR #76 review round 6, scratch container seeded from a pre-migration schema): every request through the affected route 503s while `make ps` stays green. The class is what matters, not the instance that measured it: `insurance_coverages` (mig 005), `roi_requests` (008) and `duplicate_review_queue` (009) all arrived by migration and are read the same unconditional way, so any environment older than those migrations carries the same latent break, and every future migration joins the class. Two partial escapes exist today and both mislead: `make seed` does pipe the schema first, but then re-runs the seed (the foot-gun in the next row), and hand-piping `db/schema.sql` works (all 14 tables are `CREATE TABLE IF NOT EXISTS` — `docs/runbook.md` §"Upgrading a database that predates a migration" is the operator entry) but skips existing tables whole, so `ALTER TABLE` migrations stay hand-applied and nothing records which migrations a database has had. A fuller mitigation shape — a `schema-apply` make target, structural tests pinning the schema/migration hand-sync, and a `/healthz` schema-completeness guard that turns the condition into an unhealthy container — was built and verified on the closed PR #76 branch and is cherry-pickable from it; **none of it is on `main`**. Closing the gap outright is a real runner (Alembic, or a versions table plus an apply step) — estate-wide, inside the `docs/landmines.md` §1 migrations zone | OPEN — found 2026-08-13; no runner, no detection on `main` |

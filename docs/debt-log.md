...
| D4b | `services/intake-service/app.py:143` | Eligibility results are never persisted. `_create_coverage` builds the `insurance_coverages` row without `status` or `verified_at`, and no code path anywhere updates either afterwards, so the row keeps its `'unknown'` column default permanently — even when the payer answered `active` during that same registration. The answer exists only in the `POST /intake` response body and in the ad-hoc `GET /eligibility` re-check, both of which die on the screen that rendered them. Business risk: no later shift, and nothing downstream, can see that coverage was ever confirmed, so the desk re-queries the payer for a patient already verified, and a copay decision has no record behind it. Distinct from D4, which is about *when* the check runs, not whether its answer is kept | — | OPEN |
...
| Clinician own-day schedule view unserved | The stated clinician need (`docs/design/01-operators-and-tasks.md`) is *their own* day's appointments. `GET /schedule` cannot serve it: appointments store only a provider *name*, and the `slots` join is an unenforced reference (PR #26 r1), so a by-provider filter would silently drop rows. The day queue is therefore gated front-desk/admin-only (`schedule.day_queue.read`, ADR 0017 amendment 2026-08-02, `RBAC-R11`) and clinicians have **no schedule view at all** — pressure to re-widen the role map is the risk. A real fix needs durable provider identity on appointments: a migration, belonging with the RIV-175 slot/appointment rework (W5). The split only binds migrated rows: a clinician account still on the compat `staff` role holds every capability and reaches the queue regardless (rbac.md §8 #1). | OPEN — do not close by re-granting `schedule.day_queue.read` to `clinician` |

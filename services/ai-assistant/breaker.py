"""
In-process circuit breaker for ai-assistant's call to eligibility-service.

Third copy of the same breaker (services share code by copy-paste — no shared
library, ADR 0001): eligibility-service's guards the *payer* call, intake's
guards the *intake worker thread*, and this one guards the *chat worker thread*
(ADR 0011). One seam per caller, each with independent state, because each
caller pays a different cost for the same dependency being slow.

Why the chat endpoint needs its own (ADR 0011 §4): a per-request timeout caps one
turn's worker-hold, but a chat invites retries — a clerk who gets "could not
confirm" types again — so under a sustained payer/eligibility degradation every
turn would pay the full timeout. Once this breaker opens, the outbound call is
skipped entirely and the turn answers "unconfirmed" immediately. This is the same
D4 lesson (RIV-088/RIV-141) applied to the new caller rather than re-learned.

Per-worker state only (no shared store) — see ADR 0010's tradeoff discussion. The
breaker keys nothing on the member/insurance id and its exception carries state
only, never str(e) of a downstream error, so nothing PHI-adjacent can leak
through this module.
...
class EligibilityBreakerOpen(Exception):
    """The circuit is open; the eligibility call was skipped without egress."""
...
class CircuitBreaker:
    ...
    def before_call(self) -> None:
        ...
        `_probe_in_flight` set and wedges the breaker shut. Callers guarantee this
        with try/finally (see eligibility_client.check_coverage) — the wedge was a
        real defect in an earlier round of PR #11, found in check.py after intake's
        copy had already been fixed.
        """
        with self._lock:

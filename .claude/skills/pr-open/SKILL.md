...
**Keep `@codex-review` OUT of the PR body. Post it as a separate comment
immediately after `gh pr create`** (`gh pr comment <N> --body '@codex-review'`),
as its own comment containing nothing else. Corrected 2026-08-02: the trigger
in the body **does not fire** — the bot watches comments, not the description.
The earlier "end the body with it" rule (PR #4) was wrong and cost silent
no-op rounds; PR #29 is the worked case.

Two steps, not one, and the comment is not optional garnish — without it no
review round happens at all.
...
- Confirm the `@codex-review` comment posted, by reading the PR's comments back
  (`gh pr view <N> --json comments`) rather than trusting the create call. On a
  prose-only PR, confirm the opposite — that no such comment exists — and state
  the exemption in the approval gate so the user can override.

...
Every round means the reviewer's cost is paid repeatedly. Spawn the
**`diff-reviewer`** agent per `/verify-stack` §6 — it assembles its own
briefing pack, and §6 owns the pack spec and the doctrine. The part that stays
this thread's responsibility: the spawn prompt names the branch and nothing
else. **Facts, not verdicts** — never include why the fix was chosen or what
was already checked, since inheriting this thread's assumptions is exactly what
destroys the pass's value.

`/security-review` runs only when a round earns it — see `verify-stack` §6 for
the new-surface trigger rule, for the commit-first gotcha that silently narrows
what that command reviews, and for how both lenses share one pack.

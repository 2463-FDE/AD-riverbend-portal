...
The xfail is `tests/test_hl7_parser.py::test_allergies_and_medications_are_captured`
— the RIV-160/D6 data-loss gap (AL1/RXA dropped). It staying xfail is correct;
it flipping to XPASS means parser behavior changed while the test still says the
gap is open — stop and flag, never adjust the test. (Corrected 2026-08-06: this
file used to say the xfail was the IDOR test, but that test is
`integration`-marked and sits among the *deselected*, not the xfailed.)
Deselected counts the `integration`-marked tests under `tests/integration/`:
...
`EXPECT_FAILED` in the `Makefile`, above the `test` target** — deliberately
not restated here (§10.1). Restating them is what let the deselected count sit
wrong in this file for a whole PR. Read the Makefile for the current values; a
...
**Enforcement lives in CI + branch protection on `main`** (since 2026-08-06;
the pre-push `xfail-invariant.sh` hook that ran the suite locally was retired
with the rest of the shell-text guard layer — archive/pipe1-hooks-r5). CI's
`tests` job runs this same suite, `main` requires it green plus one approving
review, and force-push is blocked — so a red push is a red check on the PR, not
a hazard. The counted xfail is `strict=True`, so an XPASS turns the CI run red
by itself. What CI does **not** assert is the deselected count and the xfail
*identity* — checking those against the Makefile constants when you run the
suite here is this ritual's job, and a drift is a stop-and-report as above.
...
**Division of labor.** You define what "proof" means for the PR — which refs,
which files revert to them, which targeted tests run, and the exact red count
each layer must produce — and then you execute it yourself, serially, in a
throwaway worktree. (The parallel `regression-proof` workflow that used to
execute the definition was retired 2026-08-06: its worktrees materialized the
default-branch tip rather than the session branch, and the recovery choreography
cost more than the parallelism bought. It survives on archive/pipe1-hooks-r5.)
Gathering the refs/files/tests/expected-reds is the judgment work; expected reds
are **measured, not guessed** — measure them in the same worktree shape the
proof runs in.

**Default path — serial, in a throwaway worktree at an explicit SHA:**

1. **Commit the round's work first**, so the proof runs against exactly the
   tree being shipped.
2. ```bash
   git worktree add ../riverbend-proof <sha>     # under Riverbend/, per §10.4
   ```
3. In the worktree: revert the layer (`git checkout <ref> -- <files>`) or apply
   the declared-bound mutation (exact file+line edit), then run the targeted
   tests — `make test-docker ARGS="tests/test_x.py -q"` from the worktree
   (`Dockerfile.test` bakes only requirements-dev.txt and bind-mounts the repo,
   so the build is a cache no-op) — and count reds.
4. Reset the worktree (`git checkout <sha> -- .`) and repeat for the next
   layer. Layers run one at a time; the layer meanings below are unchanged.
5. **Any layer whose actual red count differs from expected is a hard fail of
   the whole proof** — no partial credit. Fix the test or re-measure honestly;
   never tune the expected count to whatever came back.
6. **Cleanup:** `git worktree remove --force ../riverbend-proof`, confirm with
   `git worktree list`.

**Stash fallback** — for dirty-tree cases only, when the round's state cannot
